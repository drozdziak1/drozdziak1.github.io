...
* **Basic grasp on GNU Make** - The classic Makefiles still remain the
  kernel's build system of choice
...
message about the location of your warm, freshly built kernel - something
similar to this:
...
```
*The `$(uname -m)` parts change into your architecture's name; to exit the
VM, press `Ctrl+a` and then `x`*

If you got QEMU correctly installed, you should a bunch of kernel log
entries going through your terminal. Then it will most likely stop after a
couple seconds with...
...
Let's peek into the log excerpt from above and figure out what it means and why
...
> stuff like whether to turn on debugging, network configuration or which
> filesystem to use as the root
...
This is where BusyBox comes in. Let's head over to its directory and create the
...
program's binary. To do that, start the "graphical" configuration tool:
...
Then, use your arrows to go down and select the `Build BusyBox as a static
binary (no shared libs)` using space (an asterisk should show on the left).
...
ldd ./busybox
```
*`ldd` is a tool for checking a binary's dynamic dependencies*

If you got the config right, ldd should fend you off with a `not a dynaimc
executable` message. Looks like we're almost done! Let's go and see about that
filesystem image.
