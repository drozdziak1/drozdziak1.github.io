...
is yet another. Linux kernel modules are just pieces of binary that can get
...
some more error codes and their meaning, see [this header][errno-h].
...
│              shortly
│
└── modules.order - Contains the order in which your module's dependencies
                    should be met
```
...
`insmod` and `rmmod` (usually provided by the `kmod` package, present in the VM
thanks to `busybox`):


* insmod/rmmod console logs
* lsmod
* modprobe
* modinfo
* out-of-tree module taints kernel

# Waaah! I can't see my message!
* loglevels
