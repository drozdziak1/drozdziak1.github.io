...
modules. Then, we want `linux/kernel.h` for `printk()` and the `KERN_INFO` log
level in order to be able to write to the kernel log.
...
If you've got enough a grasp on Makefiles you'll quickly see that 
