...
For a full explanation of the preferred coding style see [the
docs][coding-style].

# The code
At last, the source. Here's what my module looks like:
```c
...
First, we include a header essential for all Linux kernel modules - `module.h`.
Then, we want `kernel.h` for the log level macros used by `printk` for grouping
log information by level of severity/importance
